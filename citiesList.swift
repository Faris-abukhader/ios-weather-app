        ...
        VStack(spacing:0){
            ZStack(alignment: .bottom){
                ScrollView(showsIndicators: false){
                    ...
                    listItem(cityName: userdata.currentCity, degree: String(Int(userdata.Data?[0].current.temp ?? 0)),icon: userdata.Data?[0].current.weather[0].icon ?? "").padding(.bottom,-18)
                    ...
                    ForEach(Array(cities.enumerated()),id:\.element){ index,city in
                            ...
                            listItem(cityName: city.name!, degree: String(Int(userdata.Data?[index+1].current.temp ?? 0)),index: index, icon:userdata.Data?[index+1].current.weather[0].icon ?? "")
                                .padding(.bottom,-18)
                            .onTapGesture {
