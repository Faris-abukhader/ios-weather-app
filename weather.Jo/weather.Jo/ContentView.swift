...
struct ContentView: View {
    ...
    var body: some View {
        ZStack{
            if userdata.Data?.count ?? 0 > 0 && threeSecond == 0 {
        VStack(spacing:0){
...
struct headMainInfo:View{
    ...
    var body: some View{
        VStack(alignment:.center){
            ...
            Text("\(Int(userdata.Data?[userdata.citiesIndex].current.temp ?? 0))º")
                .fontWeight(.light)
                .frame(width: 110)
                .font(.custom("", size: 70))
                .offset(x:10)
            
            Text("H:\(Int( userdata.Data?[userdata.citiesIndex].daily[0].temp.max ?? 0))º L:\(Int(userdata.Data?[userdata.citiesIndex].daily[0].temp.min ?? 0))º")
            ...
            ScrollView(.horizontal, showsIndicators: false){
                HStack{
                    ...
                    ForEach(0..<24){index in
                        ...
                        hourDegree(time:getDateFormat(intervalTime: Double(userdata.Data?[userdata.citiesIndex].hourly[index].dt ?? 0), dateFormat: "hh a",timeZoneIdentifier:userdata.locationTimeZone)  , icon:getIcon(icon: userdata.Data?[userdata.citiesIndex].hourly[index].weather[0].icon ?? "") , degree: Int(userdata.Data?[userdata.citiesIndex].hourly[index].temp ?? 0))
                    }
                    }
        ...
        VStack{
            if userdata.Data?.count ?? 0 > 0 {
                ForEach(0..<(userdata.Data?[userdata.citiesIndex].daily.count)!){index in
                    dayRow(day: getDateFormat(intervalTime: Double(userdata.Data?[userdata.citiesIndex].daily[index].dt ?? 0),dateFormat: "EEEE" ,timeZoneIdentifier:userdata.locationTimeZone), statusIcon: getIcon(icon: userdata.Data?[userdata.citiesIndex].daily[index].weather[0].icon ?? ""), heighestDegree: Int(userdata.Data?[userdata.citiesIndex].daily[index].temp.max ?? 0), lowestDegree: Int(userdata.Data?[userdata.citiesIndex].daily[index].temp.min ?? 0))
            }
        }
    }
        Path{path in
...
struct description:View{
    ...
    var body: some View{
        if userdata.Data?.count ?? 0 > 0 {
        Text("Today:\(userdata.Data?[userdata.citiesIndex].current.weather[0].weatherDescription ?? "") courrently.It's \(Int(userdata.Data?[userdata.citiesIndex].current.temp ?? 0))º; the high today was forest as \(Int(userdata.Data?[userdata.citiesIndex].daily[0].temp.max ?? 0))º")
